...
vars* SearchID(node *id,symbolTable *s,functions *globalfuncs)
{
  ...
  if (s1 != NULL)
  {
    while(s1!=NULL)
    {
      for (int i = 0; i < s1->VarraySize; i++)
      {
        for (int j = 0; j < s1->varsArray[i].numOfVars; j++)
        {
          //printf("EQUAL: %s %s\n",s1->varsArray[i].var_names[j],id->token);
          if(strcmp(s1->varsArray[i].var_names[j],id->token)==0)
          {
            //printf("%s\n",id->token);
            return &s1->varsArray[i];
          }
        }
      }
      s1 = s1->Sref;
...
functions* SearchFUNC(char *fname,functions *f,functions *globalfuncs,int flag)
{
  ...
  while(temp != NULL)
  {
    if(strcmp(fname,temp->fName)==0)
      return temp;
    f = temp;
    for (int i = 0; i < temp->tbl->FarraySize; i++)
    {
      if(strcmp(fname,temp->tbl->funcsArray[i].fName)==0)
      return &temp->tbl->funcsArray[i];
    }
    temp = temp->ref;
  }
  int index;
